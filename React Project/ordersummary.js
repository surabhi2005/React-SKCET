...
const OrderSummary = () => {
  ...
  return (
    <Box p={3}>
      ...
      <Typography variant="h4" mt={3} mb={2}>Review Your Order</Typography>
      <Typography>Total Amount: Rs.{total.toFixed(2)}</Typography>
      <Typography>GST (18%): Rs.{gst.toFixed(2)}</Typography>
      <Typography>Platform Fee: Rs.{platformFee.toFixed(2)}</Typography>
      <Typography variant="h5" mt={2}>Grand Total: Rs.{grandTotal.toFixed(2)}</Typography>
      ...
      <SummaryButton fullWidth sx={{ mt: 2 }} onClick={handlePayment}>Pay Now</SummaryButton>

      <Snackbar open={snackbarOpen} message="Payment Successful!" autoHideDuration={3000} />
    </Box>
  );
};
...
//     }
//     setSnackbarOpen(true);
//     setTimeout(() => navigate('/'), 3000); // Redirect to home after payment
//   };
